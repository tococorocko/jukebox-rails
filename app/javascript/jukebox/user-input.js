...
function nextSong() {
  ...
  fetch(`/next-song/${jukeBoxId}`).catch((error) => {
...
function playOrPauseSong() {
  let jukeBoxId = document.getElementById("container").getAttribute("data-jukebox-id");
  fetch(`/control-playback/${jukeBoxId}`).catch((error) => {
    console.error("Error:", error);
  });
}
...
function songSelection(e) {
  ...
  if (userInput.char != "" && userInput.num != "") {
    ...
    let songId = key.getAttribute("data-song-id");
...
function showVideo() {
  ...
  navigator.mediaDevices
    .getUserMedia({ audio: false, video: { deviceId: { exact: camera_id } } })
